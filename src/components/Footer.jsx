...
const Footer = () => {
  ...
  return (
    <footer className="mt-20 border-t border-(--border-color) bg-(--header-bg)">
      <div
        className="
    mx-auto grid max-w-7xl gap-10 px-6 py-14
    md:grid-cols-[1.3fr_1fr_1fr_0.8fr]
  "
      >
          ...
          <nav className="flex flex-col gap-2 text-sm">
            {footerNav.map(([label, path]) => (
              <Link
                ...
                to={path}
                className="
                  relative w-fit opacity-75 transition-all
                  hover:opacity-100
                  after:absolute after:left-0 after:-bottom-0.5
                  after:h-px after:w-0 after:bg-current after:opacity-40
